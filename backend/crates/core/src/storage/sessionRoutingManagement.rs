...
impl Storage {
    ...
    pub fn listRoutingSessions(&self, page: i64, search: &str) -> Result<Value> {
        let page = page.max(1);
        let count: i64 = self.conn.query_row(
            "SELECT COUNT(*) FROM session_routing_bindings WHERE instr(session_id,?1)>0",
            [search],
            |r| r.get(0),
        )?;
        let mut statement = self.conn.prepare("SELECT b.session_id,b.account_id,a.label,CASE WHEN b.account_id IS NULL AND b.status='active' THEN 'pending' ELSE b.status END,b.created_at,b.last_used_at,CASE WHEN b.account_id IS NULL AND b.status='active' THEN 'account_deleted' ELSE b.reason END,b.requested_account_id,target.label FROM session_routing_bindings b LEFT JOIN accounts a ON a.id=b.account_id LEFT JOIN accounts target ON target.id=b.requested_account_id WHERE instr(b.session_id,?1)>0 ORDER BY b.last_used_at DESC,b.session_id LIMIT 50 OFFSET ?2")?;
        let rows = statement.query_map(params![search,(page-1).saturating_mul(50)], |r| Ok(json!({
            ...
            "requestedAccountId":r.get::<_,Option<String>>(7)?,"requestedAccountLabel":r.get::<_,Option<String>>(8)?,
        })))?;
        ...
        Ok(json!({"items":items,"total":count,"page":page,"pageSize":50,"accounts":choices}))
    }
