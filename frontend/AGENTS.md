...
This file applies to `frontend/`: the Next.js frontend, the Tauri desktop shell, and
...
- Validate significant frontend changes with `pnpm run build:desktop` from this
  directory, or `pnpm -C frontend run build:desktop` from the repository root.
...
- Tauri command/lifecycle changes usually need Rust validation from the repo
  root with `cargo test --manifest-path frontend/src-tauri/Cargo.toml --lib`;
  shared service changes also require the backend workspace tests.
