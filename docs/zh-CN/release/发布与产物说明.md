...
1. 确认根 `backend/Cargo.toml`、`frontend/package.json`、Tauri 的 `backend/Cargo.toml` / `tauri.conf.json` 及两个 `Cargo.lock` 中的项目版本已同步
2. 确认 `CHANGELOG.md` 已更新
3. 确认桌面端前端构建通过：`pnpm -C frontend run build`
4. 确认核心测试通过：`pnpm -C frontend run test`、`cargo test --manifest-path backend/Cargo.toml --workspace`
5. 若涉及网关协议改动，额外执行 `backend/scripts/tests/gateway_regression_suite.ps1`
