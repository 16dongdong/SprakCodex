...
# codexmanager-web 默认会将 frontend/out 打进二进制；先构建前端即可
pnpm -C frontend run build
cargo build --manifest-path backend/Cargo.toml -p codexmanager-web --release
...
- Desktop 缓存使用 `frontend/src-tauri` 工作区，Service 缓存使用根工作区，分别保存，避免不同依赖图相互清理缓存。
- 输入：
...
会同步更新：
- 根 `backend/Cargo.toml` 的 workspace 版本
