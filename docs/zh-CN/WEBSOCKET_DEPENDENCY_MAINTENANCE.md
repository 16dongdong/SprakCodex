...
Responses WebSocket 由服务端工作区和 `frontend/src-tauri` 独立工作区共同编译。两个工作区必须使用同一组依赖来源：
...
2. 同步修改根 `backend/Cargo.toml`、`frontend/src-tauri/Cargo.toml`、`Cargo.lock` 和 `frontend/src-tauri/Cargo.lock`。
3. 执行 `bash backend/scripts/ci/check-websocket-pins.sh`，确认两个工作区的 patch 与 lockfile 一致。
4. 执行 `cargo fmt --manifest-path backend/Cargo.toml --all -- --check`、Responses WebSocket 定向测试、`cargo test --manifest-path backend/Cargo.toml --workspace --no-fail-fast`。
5. 安装前端依赖后执行 `pnpm -C frontend run build:desktop`、`cargo test --manifest-path backend/Cargo.toml -p codexmanager-web --no-fail-fast`，再执行至少一个 Tauri 目标的 bundle 构建。
...
- 根工作区格式检查、`cargo check --manifest-path backend/Cargo.toml --workspace --all-targets` 和 service WebSocket 回归测试；
