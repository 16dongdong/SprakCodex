...
- 第十三版扩展内存配置为出口环境画像：宿主经实际观测出口读取国家与 IANA 时区，映射为 Windows 动态时区键；目标进程四个时区／区域 API 与推理请求地区头共用该画像，卸载时恢复全部入口。
...
后续章节保留各历史阶段的原始验收命令和版本信息，用于回溯迁移前行为；当前发布边界以上述第十三版环境画像协议与第十二版看门狗结果为准。
