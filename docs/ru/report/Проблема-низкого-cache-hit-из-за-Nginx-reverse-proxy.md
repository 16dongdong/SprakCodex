...
Если SprakCodex развернут в Docker или как отдельный service и перед ним стоит Nginx с настройками по умолчанию, скрытая проблема обычно не в том, что "сломался кеш модели". Реальная причина в том, что **Nginx отбрасывает пользовательские заголовки с символом подчеркивания**.

SprakCodex использует эти заголовки для стабильной привязки диалога к одному thread anchor:
...
SprakCodex поддерживает совместимые endpoints `/v1/images/generations` и `/v1/images/edits`. Для этого пути риски proxy отличаются от обычных текстовых запросов:
...
Не обязательно. SprakCodex логирует usage так, как его возвращает upstream. Если thread anchor потерян еще до попадания запроса в service, backend может только зафиксировать деградировавший результат.
