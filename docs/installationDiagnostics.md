...
1. 旧版本资源被仍在运行的程序占用：先结束当前工作，正常退出相关客户端与管理程序，再安装。
...
当前尚未配置代码签名证书，发布文件未宣称已获得受信任签名。后续签名应使用维护者的有效代码签名证书，并对更新器与主程序一并签名。签名和误报复核不保证所有产品立即消除告警。

观测载荷随主程序构建并驻留于 EXE，不再作为安装目录 DLL 分发。截图来自另一台设备，本机的防护记录不能替代那台设备的调查结果。
