...
- `frontend/src/main.js`：前端启动装配入口
- `frontend/src/runtime/app-bootstrap.js`：界面初始化编排
- `frontend/src/runtime/app-runtime.js`：刷新流程与运行期协同
- `frontend/src/settings/controller.js`：设置域门面，继续向子模块分发

### 3.2 桌面端壳层入口

- `frontend/src-tauri/src/lib.rs`：Tauri 应用装配入口
- `frontend/src-tauri/src/settings_commands.rs`：桌面端设置桥接命令
- `frontend/src-tauri/src/service_runtime.rs`：桌面内嵌 service 生命周期
- `frontend/src-tauri/src/rpc_client.rs`：桌面端 RPC 调用基础设施
...
- 提供 Web UI 静态资源
- 挂载或代理到 service
- 可选把 `frontend/dist` 内嵌到二进制，形成单文件发布物
...
1. `frontend/src-tauri/src/lib.rs` 仍偏厚，桌面壳层装配与命令实现尚需继续拆开。
2. `backend/crates/service/src/lib.rs` 配置、运行时同步、副作用边界不够清晰。
3. `backend/crates/service/src/gateway/protocol_adapter/response_conversion.rs` 兼容分支较多，回归风险高。
...
- 新页面或前端交互：优先落在 `frontend/src/views/`、`frontend/src/services/`、`frontend/src/ui/`
